        ...
        else if (gMemorymode == TRUE)
        {
            if ((gSavedReadingCnt==0) && (gOneReadingDisplyed==0))
            {
               // LCD_PWR = LCD_POWER_ON;        // power up the lcd controller
                ...
                gotoXY(0,1);
                sprintf(ATime," Time= %02d:%02d:%02d", Hours, Minutes,Seconds );
                LCDString(ATime);
...
//                gTemp = gTemp* TEMPSCALE;
 ...
 // uncomment above code for reading temperature
                LCDClear();
                //char lcdFormatted = {0};
                sprintf(bGmgdate,"%03dmg/dl%02d/%02d/%02d", gYint, Month, Days, Year);
                LCDString(bGmgdate);
               // printf("%03dmg/dl%02d/%02d/%02d", gYint, Month, Days, Year); // display glucose reading in mg/dl along with date
                gotoXY(0,1);
                //lcdFormatted = {0};
                sprintf(ADCval,"ADC = %d    ",gcaptured2SValue);
                LCDString(ADCval);
                sprintf(bGmmoltime,"%d.%dmmol/L %02d:%02d", gYmmolchar,gYmmoldecimalchar, Hours, Minutes);
                LCDString(bGmmoltime);
    ...
    if (PIR1bits.ADIF == 1)
    {
       ...
       gReadData += ADRESH << 8;
        ...
        if(gReadData >= CAPTURE_START_VOLTAGE)                           // Threshold Value of 600mV checked here
        {
           ...
           {
               LATCbits.LATC7 = 0;
           } // uncomment above code for debug to test waiting period for ADC capture
           
           if(gcounter <= 0)
           {
               gcounter = 0;
               LATCbits.LATC7 = 1; // uncomment for debug to test waiting period for ADC capture
           }
